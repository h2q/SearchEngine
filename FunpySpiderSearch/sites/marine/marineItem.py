...
from FunpySpiderSearch.sites.marine.es_marine import marineIndex
...
es_marine_blog = connections.create_connection(marineIndex)
...
class marineItemLoader(ItemLoader):
    default_output_processor = TakeFirst()


class marineItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    """
    轮机工程学院Item，命名规范: 域名+内容+Item
    """
...
##            nums = 0
    ...
    def save_to_mysql(self):
        self.clean_data()
        insert_sql = """insert into marine(title,url,url_object_id,content,crawl_time) VALUES(%s,%s,%s,%s,%s)
    ...
    def save_to_es(self):
        ...
        # 在保存数据时必须传入suggest
        blog.suggest = generate_suggests(es_marine_blog, marineIndex,
                                         ((blog.title, 10), (blog.tags, 6), (blog.content, 4)))
        real_time_count('marine_blog_count', marine_COUNT_INIT)
        blog.save()
