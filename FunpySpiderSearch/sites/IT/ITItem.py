...
from FunpySpiderSearch.sites.IT.es_IT import ITIndex
...
es_IT_blog = connections.create_connection(ITIndex)
...
class ITItemLoader(ItemLoader):
    default_output_processor = TakeFirst()


class ITItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    """
    ...
    def save_to_mysql(self):
       # self.clean_data()
        insert_sql = """insert into it(title,url,url_object_id,content,crawl_time) VALUES(%s,%s,%s,%s,%s)
    ...
    def save_to_es(self):
      ...
      #  self.clean_data()
        ...
        # 在保存数据时必须传入suggest
        blog.suggest = generate_suggests(es_IT_blog, ITIndex,
                                         ((blog.title, 10), (blog.tags, 6), (blog.content, 4)))
        real_time_count('IT_blog_count', IT_COUNT_INIT)
        blog.save()
