...
from FunpySpiderSearch.sites.ms.es_ms import msIndex
...
es_ms_blog = connections.create_connection(msIndex)
...
class msItemLoader(ItemLoader):
    default_output_processor = TakeFirst()


class msItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    """
    工商学院Item，命名规范: 域名+内容+Item
    """
...
##            nums = 0
    ...
    def save_to_mysql(self):
        self.clean_data()
        insert_sql = """insert into ms(title,url,url_object_id,content,crawl_time) VALUES(%s,%s,%s,%s,%s)
    ...
    def save_to_es(self):
        ...
        blog.suggest = generate_suggests(es_ms_blog, msIndex,
                                         ((blog.title, 10), (blog.content, 4)))
        # , (blog.tags, 6)
        real_time_count('ms_blog_count', ms_COUNT_INIT)
        blog.save()
