...
from FunpySpiderSearch.sites.szb.es_szb import szbIndex
...
es_szb_blog = connections.create_connection(szbIndex)
...
class szbItemLoader(ItemLoader):
    default_output_processor = TakeFirst()


class szbItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    """
    马克思主义学院Item，命名规范: 域名+内容+Item
    """
...
##            nums = 0
    ...
    def save_to_mysql(self):
        self.clean_data()
        insert_sql = """insert into szb(title,url,url_object_id,content,crawl_time) VALUES(%s,%s,%s,%s,%s)
    ...
    def save_to_es(self):
        ...
        # 在保存数据时必须传入suggest
        blog.suggest = generate_suggests(es_szb_blog, szbIndex,
                                         ((blog.title, 10), (blog.content, 4)))
        # , (blog.tags, 6)
        real_time_count('szb_blog_count', szb_COUNT_INIT)
        blog.save()
