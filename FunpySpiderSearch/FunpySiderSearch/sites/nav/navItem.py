...
from FunpySpiderSearch.sites.nav.es_nav import navIndex
...
es_nav_blog = connections.create_connection(navIndex)
...
class navItemLoader(ItemLoader):
    default_output_processor = TakeFirst()


class navItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    """
    航海学院Item，命名规范: 域名+内容+Item
    """
...
##            nums = 0
    ...
    def save_to_mysql(self):
        self.clean_data()
        insert_sql = """insert into nav(title,url,url_object_id,content,crawl_time) VALUES(%s,%s,%s,%s,%s)
    ...
    def save_to_es(self):
        ...
        blog.suggest = generate_suggests(es_nav_blog, navIndex,
                                         ((blog.title, 10), (blog.content, 4)))
        # , (blog.tags, 6)
        real_time_count('nav_blog_count', nav_COUNT_INIT)
        blog.save()
