...
from FunpySpiderSearch.sites.cj.es_cj import cjIndex
...
es_cj_blog = connections.create_connection(cjIndex)
...
class cjItemLoader(ItemLoader):
    default_output_processor = TakeFirst()


class cjItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    """
    财经学院Item，命名规范: 域名+内容+Item
    """
...
##            nums = 0
    ...
    def save_to_mysql(self):
        self.clean_data()
        insert_sql = """insert into cj(title,url,url_object_id,content,crawl_time) VALUES(%s,%s,%s,%s,%s)
    ...
    def save_to_es(self):
        ...
        blog.suggest = generate_suggests(es_cj_blog, cjIndex,
                                         ((blog.title, 10), (blog.content, 4)))
        # , (blog.tags, 6)
        real_time_count('cj_blog_count', cj_COUNT_INIT)
        blog.save()
