...
import re
from FunpySpiderSearch.sites.cj.cjItem import cjItem, cjItemLoader
...
class msSpider(scrapy.Spider):
    ...
    url = "http://cjc.jmu.edu.cn/index.htm"
    start_urls = ['http://cjc.jmu.edu.cn/index.htm']
    other_urls =['http://cjc.jmu.edu.cn/tzgg.htm',
                'http://cjc.jmu.edu.cn/xyxw.htm',
                'http://cjc.jmu.edu.cn/kydt.htm',
                'http://cjc.jmu.edu.cn/jxgl.htm',
                'http://cjc.jmu.edu.cn/yjsjy.htm',
                'http://cjc.jmu.edu.cn/xsgz.htm'
    ]

    def parse(self, response):  # 得到所有的菜单栏链接
        new_menus = []
        selector = scrapy.Selector(response)
        menus = selector.xpath("//table[@class='flyoutMenu']//a/@href | div[@class='menuitems1']//a/@href").extract()
        for menu in menus:
            print(menus)
            if "http" not in menu:
                menu1 = self.url + menu
    ...
    def parse_content(response):
        ...
        cj_item = cjItem()
        # 通过item loader加载item
        item_loader = cjItemLoader(item=cj_item, response=response)

        selector = scrapy.Selector(response)
        title = selector.xpath("//div[@class='content-title']//td/text()").extract()[0]  # 得到页面的标题
        time = selector.xpath("//div[@class='pubdata']//td/text()").extract()[0]  # 得到页面的发布时间
      #  print("*")
      #  print(time)
